...
- Loopback first: the Gateway WS defaults to `ws://127.0.0.1:18789`. The wizard generates a gateway password by default, even for loopback. For tailnet access, run `openclaw gateway --bind tailnet --auth password --password ...` because non-loopback binds require a password unless you use proxy auth.
