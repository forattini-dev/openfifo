...
export async function resolveGatewayRuntimeConfig(params: {
  ...
  const hasProxyAuth = authMode === "proxy";
  const hooksConfig = resolveHooksConfig(params.cfg);
  const canvasHostEnabled =
    process.env.OPENCLAW_SKIP_CANVAS_HOST !== "1" && params.cfg.canvasHost?.enabled !== false;

  assertGatewayAuthConfigured(resolvedAuth);
  if (authMode === "proxy" && trustedProxies.length === 0) {
    throw new Error(
      "gateway.auth.mode=proxy requires gateway.trustedProxies (set proxy IPs to trust auth headers)",
    );
  }
  ...
  if (!isLoopbackHost(bindHost) && !hasSharedSecret && !hasProxyAuth) {
    throw new Error(
